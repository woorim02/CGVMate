...
public class CgvEventService : CgvServiceBase
{
    ...
    {
        var body = await _client.GetStringAsync($"https://m.cgv.co.kr/WebApp/EventNotiV4/EventDetailGeneralUnited.aspx?seq={eventId}");
        var document = new HtmlDocument();
        document.LoadHtml(body);
        var imgNodes = document.DocumentNode.SelectNodes("//div[@class='wrap_box ir']/img");
        var list = new List<string>(imgNodes.Count);
