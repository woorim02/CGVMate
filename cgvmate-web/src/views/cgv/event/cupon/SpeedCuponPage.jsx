  ...
  useEffect(() => {
    if (isRunning) {
      intervalRef.current = setInterval(async () => {
        const response = await api.getSpeedCuponCountsAsync();
        const selectedCupons =
          selectedMovieIndex === 'nullmoviewqwegv' ? response : response.filter((x) => x.movieIndex === selectedMovieIndex);
    ...
    <Container>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        ...
        <FormControl variant="outlined" sx={{ minWidth: 150 }}>
          ...
          <Select labelId="movie-select-label" value={selectedMovieIndex} onChange={handleMovieIndexChange} label="영화 선택">
            <MenuItem value="nullmoviewqwegv">All</MenuItem>
            {cupons.map((cupon, index) => (
              <MenuItem key={index} value={cupon.movieIndex}>
                {cupon.movieTitle}
              </MenuItem>
            ))}
          </Select>
